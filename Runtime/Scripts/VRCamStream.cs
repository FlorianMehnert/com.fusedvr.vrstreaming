...
namespace FusedVR.VRStreaming
{
    public class VRCamStream : VideoStreamSender
    {
        ...
        [SerializeField] [Tooltip("The Cameras to combine together for the render texture")]
        private Camera[] cameras;

        [SerializeField] [Tooltip("Defines the depth buffer used for render streaming (0, 16, 24, 32)")]
        private int depth = 0;

        [SerializeField] [Tooltip("Defines the number of samples for anti-aliasing (1, 2, 4, 8)")]
        private int antiAliasing = 1;

        [SerializeField] [Tooltip("Defines the default bitrate to be used by the encoders in bits per second")]
        ...
        /// </summary>
        protected MediaStreamTrack CreateTrack()
        {
            RenderTextureFormat format = WebRTC.GetSupportedRenderTextureFormat(SystemInfo.graphicsDeviceType);
            RenderTexture rt = new RenderTexture((int)width, (int)height, depth, format)
            {
        ...
        {
            // The 'Senders' dictionary was removed in Unity Render Streaming 3.1.0-exp.9.
            // Bitrate and framerate should now be configured directly on the VideoStreamSender 
            // component in the Unity Inspector, or via its native properties.
        }
